...
(define do-boolean-stuff-toaster
  (lambda (op num1 num2)
    (cond
      ((eq? op '<) (< num1 num2))
      ((eq? op '>) (> num1 num2))
...
(define no-boolean-parser
  (lambda (no-code-boolean)
    (list 'boolean-exp (cadr no-code-boolean) (no-parser (caddr no-code-boolean))
...
(define run-parsed-boolean-code
  (lambda (parsed-boolean-no-code env)
    (run-parsed-code (cadr (cddr parsed-boolean-no-code)) env)))
